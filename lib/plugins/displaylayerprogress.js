        ...
        .then((response) => {
            if (response.status === 200) {
                ...
                if (response.data.layer.averageLayerDurationInSeconds !== '-') {
                    adapter.setStateAsync('plugins.displayLayerProgress.layer.averageDuration', { val: parseInt(response.data.layer.averageLayerDurationInSeconds), ack: true });
                }

                if (response.data.layer.lastLayerDurationInSeconds !== '-') {
                    adapter.setStateAsync('plugins.displayLayerProgress.layer.lastDuration', { val: parseInt(response.data.layer.lastLayerDurationInSeconds), ack: true });
                }
                ...
                adapter.log.error(`[plugin display layer progress] status ${response.status}: ${JSON.stringify(response.data)}`);
            }
        })
        .catch((error) => {

...
async function downloadThumbnailFor(adapter, filePath, thumbnailUrl) {
    try {
        ...
        if (response.data) {
            ...
            adapter.log.debug(
                `[downloadThumbnailFor] [plugin slicer thumbnails] received data as ${responseType} for ${thumbnailUrl}: ${JSON.stringify(response.headers)}`,
            );

            adapter.writeFile(adapter.namespace, filePath, Buffer.from(response.data), () => {
                adapter.log.debug(
                    `[downloadThumbnailFor] [plugin slicer thumbnails] saved file information in ${filePath}`,
                );
            });
        } else {
            adapter.log.debug(
                `[downloadThumbnailFor] [plugin slicer thumbnails] response was empty: ${JSON.stringify(response.headers)}`,
            );
        }
    } catch (error) {
        if (error.response) {
            ...
            adapter.log.warn(
                `[downloadThumbnailFor] [plugin slicer thumbnails] received ${error.response.status} response from ${thumbnailUrl}`,
            );
            ...
            adapter.log.info(
                `[downloadThumbnailFor] [plugin slicer thumbnails] error ${error.code} from ${thumbnailUrl}: ${error.message}`,
            );
        } else {
